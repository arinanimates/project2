...
var actorChars = {
  "@": Player,
  "o": Door, // A door will wobble up and down
  "=": Rain, "|": Rain, "v": Rain,
  "c" : Candy,
  "s" : Spike
};
...
function Player(pos) {
  ...
  this.size = new Vector(1, 1); //0.8, 1.5
...
// Rain is initialized based on the character, but otherwise has a
// size and position
function Rain(pos, ch) {
  ...
  if (ch == "=") {
    ...
    // Drip Rain. Repeat back to this pos.
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle Rain by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches Rain and the player hasn't won
  // Player loses
  if (type == "Rain" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "door") {
    ...
    // If there aren't any doors left, player wins
